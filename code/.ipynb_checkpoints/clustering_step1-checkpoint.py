...
def plot_families(number_families,fgv,v,plot_title):
  ...
  for F in families:
    ...
    obj['fig'].savefig('results/clustering_step1/%s_Cluster %s.png' % (plot_title,F),
...
fig.savefig('results/clustering_step1/k-means_clustering')
...
for f in range(k):
    ...
    plt.legend()
plt.savefig('results/clustering_step1/day_of_year_distribution')
...
for i in range(k):
    ...
    plt.savefig('results/clustering_step1/dendrogram_fam%s.png' % i)
    
    # save the new datasets with the F_kmeans column
    cpv_1.to_csv(path_or_buf = "results/clustering_step1/cpv_fam%s.csv" % i, index=False)
    gv_1.to_csv(path_or_buf = "results/clustering_step1/gv_fam%s.csv" % i, index=False)
